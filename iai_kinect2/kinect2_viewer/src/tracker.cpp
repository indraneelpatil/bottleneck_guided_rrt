...
pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_pass_ (new pcl::PointCloud<pcl::PointXYZRGBA>);
pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_pass_downsampled_(new pcl::PointCloud<pcl::PointXYZRGBA>);
pcl::PointCloud<pcl::PointXYZRGBA>::Ptr particle_cloud (new pcl::PointCloud<pcl::PointXYZRGBA>);
 pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_cluster (new pcl::PointCloud<pcl::PointXYZRGBA>);
 pcl::PointCloud<pcl::PointXYZRGBA>::Ptr statistical_outlier (new pcl::PointCloud<pcl::PointXYZRGBA>);
 pcl::PointCloud<pcl::PointXYZRGBA>::Ptr kpts(new pcl::PointCloud<pcl::PointXYZRGBA>); 



pcl::PointXYZ tmp; 
pcl::PointXYZRGBA tmp2; 
...
// Creating the KdTree object for the search method of the extraction
  ...
  //pcl::FeatureFromNormals<pcl::PointXYZRGBA, pcl::Normal, pcl::FPFHSignature33>::Ptr feature_from_normals = boost::dynamic_pointer_cast<pcl::FeatureFromNormals<pcl::PointXYZRGBA, pcl::Normal,pcl::FPFHSignature33> > (feature_extractor_);
...
void processing ()
{
  ...
  sor.filter (*statistical_outlier);
//cout<<"Cloud Before Filtering"<<target_cloud->points.size()<<endl;
//cout<<"Cloud After Filtering"<<statistical_outlier->points.size()<<endl;

  detector.setNonMaxSupression (true);
  detector.setInputCloud (statistical_outlier);
  ...
  std::cout << "keypoints detected: " << keypoints->size() << std::endl; 
   ...
   ne.setSearchMethod (tree);
    ...
    ne.setInputCloud (statistical_outlier);
    ne.compute (*normals);


fpfh.setInputCloud(kpts);
fpfh.setSearchSurface(statistical_outlier);
fpfh.setInputNormals(normals);
fpfh.setSearchMethod(tree);
fpfh.setRadiusSearch(0.2);
fpfh.compute(*descriptors);

cout<<"Resulting descriptors are of size :"<<descriptors->points.size()<<endl;
 ...
 ec.setClusterTolerance (0.015); // 2cm
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
  {
    ...
    for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); ++pit)
      cloud_cluster->points.push_back (target_cloud->points[*pit]); //*
...
/*
 ...
 pass.setFilterFieldName ("x");
  ...
  sor.setLeafSize (0.01, 0.01, 0.01);  //0.01  //0.005
  sor.filter (*cloud_pass_downsampled_);
...
void cloudViewer(const std_msgs::Float32ConstPtr& number)
{
   ...
   visualizer->addPointCloud(kpts, cloudName);
   visualizer->addPointCloud(statistical_outlier, "points");
  ...
  createcloud();
   ...
   visualizer->updatePointCloud(kpts, cloudName);
    visualizer->updatePointCloud(statistical_outlier, "points");
