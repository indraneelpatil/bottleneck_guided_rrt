...
def model_pass(input,endpoints,training):
	...
	concatenatedFeatures =  tf.concat([fc3,endpoints],1)   
	print(concatenatedFeatures.shape)                        #134

	fc4=fully_connected(concatenatedFeatures,size=num_keypoints,scope_name='fc4')     #9
	prediction=fc4
...
with graph.as_default():
	...
	predictions = model_pass(tf_x_batch,tf_endpoints_batch, is_training)
...
with tf.Session(graph=graph) as session:
	...
	for epoch in range(num_epochs):
		...
		for x_batch,y_end_batch in batch_iterator(voxels,labels_endpoints):
			y_batch,x2_batch=np.split(y_end_batch,[9],axis=1)
			session.run(optimizer,feed_dict={tf_x_batch:x_batch,tf_endpoints_batch:x2_batch,tf_y_batch:y_batch,is_training:True})
		...
		if(epoch%every_epoch_to_log==0):
			...
			for x_batch,y_end_batch in batch_iterator(voxels,labels_endpoints):
				...
				y_batch,x2_batch=np.split(y_end_batch,[9],axis=1)
				l=session.run(loss,feed_dict={tf_x_batch:x_batch,tf_endpoints_batch:x2_batch,tf_y_batch:y_batch,is_training:False})
				total_loss+=l
				#p.extend(p_batch)
			#p=np.asarray(p)
			#print(p.shape)
	...
	writer=tf.summary.FileWriter('./graphs/rrt_multiinput_convnet',graph)
