...
float goal_chance=0.2;
float cache_chance=0.3;
float rand_chance=0.8; //(1-goal_chance)
float beta=1;          //Adaptive beta parameter
...
int k;
 ...
 // Random Point marker shape
  ...
  std::vector<std::vector<float>> waypoint_cache;
...
std::vector<geometry_msgs::Pose> BuildPlan(std::vector< std::vector<float>> V_tree)
{
        ROS_INFO("Building the plan.");
        int number_of_waypoints=0;
        
        int current_index=(V_tree.size()-1);
        while(current_index>0)
        {
          std::cout<<current_index<<std::endl;
          index_path.push_front(current_index);
          current_index=V_tree[current_index][3];
        ...
        for(int s=0;s<index_path.size();s++)
        {
              k=index_path[s];
              //std::cout<<k<<std::endl;
              geometry_msgs::Pose pos;
              pos.position.x = V_tree[k][0];
              pos.position.y = V_tree[k][1];
              pos.position.z = V_tree[k][2];
          ...
          //trajectory_final.poses.push_back(pos);
           // Visualizing the Final Trajectory from Start to Goal 
              ...
              if(a==0)
                  {
                      //std::cout<<"Line Between :"<<index_path[s+1]<<"and "<<int(index_path[s+2])<<std::endl;
                      po.x=V_tree[k][0];     // Add the start to the trajectory
                      po.y=V_tree[k][1]; 
                      po.z=V_tree[k][2];
                      line_trajectory.points.push_back(po);
                      po.x=V_tree[index_path[s+1]][0]; 
                      po.y=V_tree[index_path[s+1]][1]; 
                       po.z=V_tree[index_path[s+1]][2];
                       line_trajectory.points.push_back(po);
                  ...
                  {
                     std::cout<<"Line Between*** :"<<prev_k<<"and "<<k<<std::endl;
                      po.x=V_tree[prev_k][0]; 
                     po.y=V_tree[prev_k][1]; 
                     po.z=V_tree[prev_k][2];
                     line_trajectory.points.push_back(po);
                     po.x=V_tree[k][0]; 
                      po.y=V_tree[k][1]; 
                     po.z=V_tree[k][2]; 
...
{   
    ...
    { 
          if(l==0)
          {
              cost=cost+beta*Norm(tree[current_index],parent);
              l++;
          ...
          else
          {
              ...
              cost=cost+beta*Norm(tree[current_index],tree[prev_index]);
              prev_index=current_index;
              current_index=tree[current_index][3];
           }
...
double distance_from_obstacle(std::vector<float> point1)
{
    ...
    Transform3f tf0;
    tf0.setIdentity();
    tf0.setTranslation(Vec3f(point1[0], point1[1],point1[2]));
    std::shared_ptr<Sphere> Shpere0(new Sphere(0.1));
    CollisionObject co0(Shpere0, tf0);
    for(size_t i = 0; i < boxes.size(); ++i)
        {
            ...
            //dist_result.update(0, NULL, NULL, fcl::DistanceResult::NONE,fcl::DistanceResult::NONE);  // can be faster
...
line_trajectory.lifetime = ros::Duration();
...
double temp_dist=distance_from_obstacle(start_command);
std::cout<<temp_dist<<std::endl;
std::cout<<" "<<std::endl;
double temp_dist2=distance_from_obstacle(goal_command);
std::cout<<temp_dist2<<std::endl;
while(ros::ok())
{
...
{
      //std::cout<<"Back to step one:"<<std::endl;
      points_temp.id = i;
      //lines_temp.id=j;
      j=j+1;
      i=i+1;
     ...
     // xr=loadedData[random_indice][4];
      ...
      if(cache_size>1)
      {
            if(rand_chance>cache_chance)
          {
            ...
            else if (rand_chance<cache_chance&&rand_chance>goal_chance)
            {
              xr=waypoint_cache[cache_indice][0];
              yr=waypoint_cache[cache_indice][1];
              zr=waypoint_cache[cache_indice][2];
              std::cout<<"Random Point from the waypoint cache!!"<<std::endl;
...
//if(found==true)
            ...
            {   
                ...
                { 
                    ...
                    //Store smooth plan in the way point cache
                    waypoint_cache.clear();
                    for(int e=0;e<smooth_plan.size();e++)
                      {
                        std::vector<float> cache_point;
                        cache_point.push_back(smooth_plan[e].position.x);
                        cache_point.push_back(smooth_plan[e].position.y);
                        cache_point.push_back(smooth_plan[e].position.z);
                        waypoint_cache.push_back(cache_point);
                        cache_point.clear();
                      }
                    std::cout<<"Waypoint cache has :"<<int(waypoint_cache.size())<<" Number of Points"<<std::endl;
      ...
      if(goal_search==false)
      {
        ...
        while(replan&&ros::ok())
        {
          ...
          for(int q=0;q<(int(simplify_plan.size()-1));q++)
            {
              //Update Octomap
              ros::spinOnce();
              std::vector<float> test1,test2;
              test1.push_back(simplify_plan[q].position.x);test1.push_back(simplify_plan[q].position.y);test1.push_back(simplify_plan[q].position.z);
              test2.push_back(simplify_plan[q+1].position.x);test2.push_back(simplify_plan[q+1].position.y);test2.push_back(simplify_plan[q+1].position.z);
              bool check_path=isLineValid(test1,test2);
              test1.clear();
              test2.clear();
                if(check_path)
                  {
                    std::cout<<"Replanning needed collision detected"<<std::endl;
                    goal_search=true;
                    replan=false;
                    found=true;
                    collision_result=true;
                    ...
                    V.clear();
                    point_new.clear();
                    point_new.push_back( start_command[0]);point_new.push_back( start_command[1]);point_new.push_back(start_command[2]);point_new.push_back(-1);  
                    V.push_back(point_new);
                    std::cout<<"Tree has been reset due to obstacle to size of :"<<int(V.size())<<std::endl;
                    points.markers.clear();
                    line_temp.points.clear();
                    line_trajectory.points.clear();
                    goal.clear();
                    goal.push_back(goal_command[0]);goal.push_back(goal_command[1]);goal.push_back(goal_command[2]);
                    plan.clear();
                    simplify_plan.clear();
                    smooth_plan.clear();
                    pruned_path.clear();
                    smoothened_path.clear();
                    index_path.clear();
                    pruned_size=0;
                    a=0;
                    prev_k=0;
                ...
                {
               //   std::cout<<"Line is safe"<<std::endl;
                  ros::Duration(0.1).sleep(); // sleep for 1/10th a second
                  continue;
                }
